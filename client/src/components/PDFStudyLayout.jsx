...
function PDFStudyLayout({ pdfUrl, pdfTitle = '', initialNotes = '', noteId: externalNoteId = null }) {
  const [notes, setNotes] = useState(initialNotes);
  const [noteId, setNoteId] = useState(externalNoteId);
  const [isSaving, setIsSaving] = useState(false);
  const dispatch = useDispatch();
  const { user } = useSelector((state) => state.auth);
  const { notes: userNotes, isLoading, isError, message, isSuccess } = useSelector((state) => state.notes);

  // Debug when props change
  useEffect(() => {
    console.log('PDFStudyLayout props:', { pdfUrl, pdfTitle, initialNotes, externalNoteId });
  ...
  useEffect(() => {
    console.log('Redux notes state:', { userNotes, isLoading, isError, message, isSuccess });
  ...
  const handleSaveNotes = async () => {
    ...
    try {
      ...
      console.log('Note data:', { title: noteTitle, content: notes.substring(0, 50) + '...', pdfId });
      ...
      if (noteId) {
        ...
        const updated = await dispatch(updateNote({ noteId, noteData })).unwrap();
